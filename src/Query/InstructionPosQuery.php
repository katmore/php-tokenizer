...
class InstructionPosQuery implements
Token\QueryInterface
{
   ...
   protected function currentMatch(Token\IteratorInterface &$iterator): ?Token {
      ...
      while ($iterator->valid()) {
         ...
         if ($token->getContext()->getInstructionPos()===$this->instructionPos) {
            $match = $token;
         }
         $iterator->next();
         if ($match !== null) {
   ...
   public function __construct(Token\IteratorInterface $iterator, int $instructionPos) {
